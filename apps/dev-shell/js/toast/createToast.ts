...
function buildContent(attributes: ResolvedToastAttributes): HTMLElement {
  ...
  if (attributes.title) {
    /* Not a heading. The alert around it is a live region — role="status" or
       role="alert" announces the whole toast — so a heading adds nothing and
       costs a phantom entry in the document outline. .alert-title refuses to
       style h1-h6 for that reason. */
...
function buildActions(): HTMLElement {
  ...
  button.className = "pu-btn btn-ghost btn-sm";
  button.type = "button";
...
export function createToast(id: string, attributes: ResolvedToastAttributes): HTMLElement {
  ...
  alert.className = `pu-alert alert-${attributes.size}`;
